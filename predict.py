...
def predict():
    ...
    if args["model"] not in model_list:
    	raise AssertionError("The --model command line argument should be one from this list [vgg16,inception,xception,resnet].")
    ...
    ts = get_timestamp()
    output_dir = get_output_directory(indexPath, ts, img_path, folder_out)
    
    # load the model and index the results
    loader = Loader( indexPath = indexPath, 
                    output = output_dir, 
                    timestamp = ts,
                    verbose = verbose )
